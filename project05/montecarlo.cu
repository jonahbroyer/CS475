...
// use PROJECT1 to check answer from project 1, then comment out
//#define PROJECT1
...
MonteCarlo( IN float *dtxs, IN float *dtys, IN float *dtxvs, IN float *dsvs, IN float *dsths, IN float *dhalflens, OUT int *dhits )
{
        ...
        // does the snowball hit the truck:
		if( fabs(sbx - truckx) < halflens)
		{
				dhits[gid] = 1;
		}
}
